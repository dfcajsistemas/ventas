                                ...
                                <td>
                                    ...
                                    @can('mantenimiento.productos.eliminar')
                                        <button x-data="eliminar" class="btn btn-icon btn-danger btn-sm"
                                            title="Eliminar"
                                            x-on:click="confirmar({{ $producto->id }}, '{{ $producto->nombre }}')"><i
    ...
    <x-modal-view mId="mDetails" :mTitle="$mTitle" mSize="md">
        <div class="table-responsive">
            @if ($prod)
                <table class="table table-sm table-hover text-small">
                    <tr>
                        <th colspan="2"><small>Nombre</small>
                            <h5 class="text-info">{{ $prod->nombre }}</h5>
                        </th>
                    </tr>
                    <tr>
                        <th>Descripción</th>
        ...
        <script>
            ...
            Alpine.data('eliminar', () => ({
                confirmar(id, nombre) {
                    Swal.fire({
                        title: '¿Estás seguro?',
                        html: "¡No podrás revertir esto!<br><b>" + nombre + "</b>",
                        icon: 'warning',
